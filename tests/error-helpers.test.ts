import { LumoxError, LumoxErrorCode, CryptoError, StorageError } from '../src/core/errors';
import {
  isLumoxErrorCode,
  isErrorCategory,
  isStorageError,
...
describe('Error Helpers', () => {
  describe('isLumoxErrorCode', () => {
    it('should identify errors with specific code', () => {
      ...
      expect(isLumoxErrorCode(new Error('Test error'), LumoxErrorCode.STORAGE_READ_ERROR)).toBe(
        false
      );
    });
  });
  ...
  describe('unwrapError', () => {
    it('should unwrap nested errors', () => {
      ...
      const cryptoError = new CryptoError(LumoxErrorCode.CRYPTO_ENCRYPTION_FAILED, 'Crypto error', {
        cause: originalError,
      });
      const lumoxError = new LumoxError(LumoxErrorCode.UNKNOWN_ERROR, 'Lumox error', {
        cause: cryptoError,
      });
  ...
  describe('getUserFriendlyErrorMessage', () => {
    it('should return user-friendly messages for error codes', () => {
      expect(getUserFriendlyErrorMessage(LumoxErrorCode.STORAGE_READ_ERROR)).toBe(
        'Failed to read data from storage.'
      );
      expect(getUserFriendlyErrorMessage(LumoxErrorCode.CRYPTO_KEY_NOT_SET)).toBe(
        'Encryption key is not set.'
      );
      expect(getUserFriendlyErrorMessage(LumoxErrorCode.INITIALIZATION_FAILED)).toBe(
        'Failed to initialize the system.'
      );
      expect(getUserFriendlyErrorMessage('UNKNOWN_CODE' as LumoxErrorCode)).toBe(
        'An error occurred.'
      );
    });
  });
